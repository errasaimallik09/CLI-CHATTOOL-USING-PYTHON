import socket#clients file
import threading
import pickle
import sys

state = {}#state dict ionary of client
#to get response from the clients given input
...
#to check wheter he is getten into grp or else disconnected
def waitServerListen(serverSocket):
	while not state["alive"]:
		...
		if msg == "/accepted":
			...
			break
#when thee user send the request and want to connect or refuse from the grp
...
def main():
	#min args length is 3 included with  port and host
	if len(sys.argv) < 3:
	...
	if response == "/adminReady":
		...
		#created thread for the client has to get access to the grp
	...
	# when the user enters the input he chooses the options he want
